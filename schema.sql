...
create table if not exists public.families (
  ...
  -- Migración 055. El país de operación, en ISO 3166-1 alfa-2. Se declara
  -- explícitamente y NUNCA se infiere: ni del idioma, ni de la IP, ni del
  -- correo, ni del dispositivo, ni —y esta es la tentación de al lado— de
  -- `timezone`. Sacar 'ES' de 'Europe/Madrid' sería un país inventado por el
  -- servidor con apariencia de dato declarado.
  --
  -- Nulo quiere decir «creado antes de la 055 y sin declarar», que es la
  -- verdad y no le impide nada de lo que ya hacía. Mismo criterio que
  -- `legal_version` en la 022.
  pais text
    check (pais is null or pais ~ '^[A-Z]{2}$'),
  pais_declarado_at timestamptz,
  -- Sin clave ajena a `auth.users` a propósito, y por lo mismo que
  -- `configuracion_expansion.publicada_por`: con `on delete set null`,
  -- borrar la cuenta dispararía un `update` que el disparador rechaza y el
  -- borrado de la cuenta fallaría. Un apunte de auditoría sobrevive a la
  -- cuenta que nombra.
  pais_declarado_por uuid,
...
-- Desde la migración 057 **NO es único**: una persona puede tener varios
-- gremios. Sigue existiendo porque `mis_gremios()` empieza por
-- `families.owner = auth.uid()` y sin él cada petición recorrería la tabla
-- entera. El párrafo de arriba explica por qué llegó a ser único, y por qué
-- dejó de serlo solo cuando el cliente dejó de cargar con `limit 1`.
...
-- ---------------------------------------------------------------------
-- MIGRACIÓN 055 · el país de operación se declara, nunca se deduce
--
-- La matriz tipo × país × estado existe desde la 050 y `tipo_publicado()` la
-- contesta; lo que faltaba era **el país**, y por eso aquella función se dejó
-- sin conceder a `authenticated`: el país es un parámetro suyo, y `R-108`
-- dice que un cliente no declara en qué país está para desbloquear un tipo.
-- Quien la llame tiene que ser una función del servidor que sepa de dónde
-- sacar el país de verdad, y el país de verdad es `families.pais`.
--
-- Ningún gremio recibe un país por inferencia (`R-102`) y **ninguno se
-- bloquea por no tenerlo** (`R-117`): no hay `not null`, no hay valor por
-- defecto, y ninguna de las funciones que hoy se llaman empieza a exigirlo.
-- Se le preguntará al gremio la primera vez que intente algo que dependa de
-- él, y eso es la Fase 5.
-- ---------------------------------------------------------------------
...
-- ---------------------------------------------------------------------
-- MIGRACIÓN 056 · la llave se forja (Fase 5)
--
-- El nivel derivado en **servidor** desde la marca de agua, el derecho de
-- expansión —la llave— con su ciclo de vida entero, y la forja con todas sus
-- comprobaciones.
--
-- Dos cosas que conviene tener delante al leer lo de abajo:
--
--   · **La fórmula del nivel vive ahora en tres sitios** (aquí, en
--     `src/lib/supabase.js` y en `fakeBackend.js`). Es inevitable desde que el
--     servidor tiene que comprobar el nivel sin fiarse del cliente (`R-26`).
--     Lo que evita que se separen es `tests/llave.test.js`, que extrae la
--     aritmética de este fichero y la compara con la de JS.
--
--   · **El límite global se mira ANTES de cobrar** (`R-61`). Cobrar por una
--     llave que no se puede usar es cobrar por nada, y por eso nada de lo que
--     hay antes del paso 11 de `forjar_llave()` toca la cartera.
-- ---------------------------------------------------------------------
...
-- ---------------------------------------------------------------------
-- MIGRACIÓN 057 · una persona, varios gremios (Fase 6.1 · el servidor)
--
-- Invitaciones con caducidad, crear un gremio con llave, aceptar con llave,
-- abandonar, expulsar y reingresar. **No toca el cliente**: mientras nadie
-- tenga dos gremios, nada de esto cambia lo que ve la familia de hoy.
--
-- Tres cosas que conviene tener delante:
--
--   · **`idx_families_owner` deja de ser único** y no desaparece: la primera
--     rama de `mis_gremios()` es `families.owner = auth.uid()`, y sin índice
--     cada petición recorrería la tabla de familias entera.
--
--   · **Un gremio creado con llave nace sin credencial compartida**, y su
--     `owner` es la cuenta personal de quien lo crea. `credenciales` prohíbe
--     que una personal lleve `family_id`, así que no hay otra forma; darle una
--     credencial compartida es la Fase 7.
--
--   · **El reingreso reactiva el personaje anterior** (`R-63`): «empezar desde
--     cero en cada gremio» es la primera vez que entras, no cada vez. Volver a
--     casa no puede costarte el historial.
-- ---------------------------------------------------------------------
...
-- ---------------------------------------------------------------------
-- MIGRACIÓN 059 · reclamar un perfil que ya era tuyo (Fase 7.1 · `F-11`)
--
-- Alguien que ya es persona en el gremio A y que además lleva meses siendo un
-- perfil interno del gremio B. No tiene que empezar de cero ni pagar una
-- llave por algo que ya era suyo: el perfil, su historial y su relación con
-- el gremio existían antes de que activara su identidad.
--
-- Tres cosas que conviene tener delante:
--
--   · **No cuesta llave, pero ocupa plaza** (`R-86`). El límite mide gremios
--     a los que se pertenece de forma activa, no la vía por la que se llegó;
--     si no ocupara, los perfiles internos serían la manera de eludir la
--     progresión y el coste de expansión.
--
--   · **Lo aprueba el gremio de destino**, no quien reclama. Sin eso,
--     cualquiera con el identificador de un perfil podría apropiárselo.
--
--   · **`solicitar_reclamacion` no distingue** «no existe» de «existe y no se
--     puede» (`SEC-9`). Distinguirlos la convertiría en un detector de
--     perfiles: se prueban identificadores hasta que uno responde distinto.
-- ---------------------------------------------------------------------
...
-- ---------------------------------------------------------------------
-- MIGRACIÓN 060 · quitar la llave de debajo del felpudo (Fase 7.2 · `F-12`)
--
-- Un gremio donde ya todo el mundo tiene su identidad no necesita una clave
-- común, y mantenerla es un correo y una contraseña que abren la casa entera
-- y no representan a nadie.
--
-- **Desactivar tiene que cortar DOS caminos, no uno.** La credencial
-- compartida entra por dos sitios distintos en `mis_gremios()`: por
-- `families.owner = auth.uid()` y por su fila de `credenciales`. Poner
-- `activa = false` cierra el segundo y deja el primero abierto de par en par.
...
-- llave desde la 057.
--
-- Y `puede()` se reescribe para lo mismo: una credencial retirada no
-- autoriza nada.
-- ---------------------------------------------------------------------
...
-- ---------------------------------------------------------------------
-- MIGRACIÓN 061 · en Equipo no se mide a nadie
--
-- La respuesta jurídica del 31-ago pone como criterio de salida del tipo
-- Equipo «una prueba de que las APIs **también** bloquean toda métrica
-- individual». Ese «también» es la palabra importante: la plantilla declara
-- `progreso_individual = false` desde la 053 y **no lo leía nadie**.
--
-- Van disparadores y no un `if` en cada función porque el criterio dice **las
-- APIs**: PostgREST expone las tablas, y una guarda que solo viviera dentro de
-- las RPC dejaría la puerta de al lado abierta.
-- ---------------------------------------------------------------------
